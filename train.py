...
def train(opt):
    ...
    dataset.train()
    train_loader = DataLoader(dataset, batch_size=opt.batch_size, shuffle=opt.shuffle, num_workers=4)
    dataset.val()
    val_loader = DataLoader(dataset, batch_size=opt.batch_size, shuffle=False, num_workers=4)
    ...
    for epoch in range(logger.epoch_start, opt.max_epochs):
        ...
        for iter, batch in enumerate(train_loader):
            print(f"Iteration {iter} in Epoch {epoch}")
            start = time.time()
            logger.iteration += 1

            feature_fc = batch['feature_fc']
            target = batch['split_story']
            if torch.cuda.is_available():
                feature_fc = feature_fc.cuda()
                target = target.cuda()

            optimizer.zero_grad()

            output = model(feature_fc, target)
            print(f"Model output calculated for Iteration {iter} in Epoch {epoch}")
            loss = crit(output, target)
            print(f"Loss calculated: {loss.item()}")

            if opt.start_rl >= 0 and epoch >= opt.start_rl:
                seq, seq_log_probs, baseline = model.sample(feature_fc, sample_max=False, rl_training=True)
                rl_loss, avg_score = rl_crit(seq, seq_log_probs, baseline, batch['index'])
                print(f"RL loss: {rl_loss.item() / loss.item()}")
                loss = opt.rl_weight * rl_loss + (1 - opt.rl_weight) * loss
                logging.info("average {} score: {}".format(opt.reward_type, avg_score))
            ...
            logging.info("Epoch {} - Iter {} / {}, loss = {:.5f}, time used = {:.3f}s".format(epoch, iter, len(train_loader), train_loss, time.time() - start))
            if logger.iteration % opt.losses_log_every == 0:
                logger.log_training(epoch, iter, train_loss, opt.learning_rate, model.ss_prob)
            ...
            if logger.iteration % opt.save_checkpoint_every == 0:
                ...
                if logger.best_val_score > score:
                    ...
                    if bad_valid >= 4:
                        ...
                        checkpoint_path = os.path.join(logger.log_dir, 'model-best.pth')
                        model.load_state_dict(torch.load(checkpoint_path))
                        utils.set_lr(optimizer, opt.learning_rate)
                        bad_valid = 0
                        logging.info("bad valid : {}".format(bad_valid))
                else:
                    logging.info("achieving best {} score: {}".format(opt.metric, score))
